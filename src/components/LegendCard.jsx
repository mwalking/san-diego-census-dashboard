...
function LegendCard({ geoMode, onGeoModeChange, activeMetricLabel }) {
  return (
    <section className="w-full rounded-xl border border-slate-200/10 bg-slate-900/85 p-4 shadow-xl backdrop-blur">
      ...
      <p className="mt-3 text-xs text-slate-300">{COPY.legend.modeSubtext[geoMode]}</p>
      <p className="mt-1 text-xs text-slate-400">{activeMetricLabel}</p>
      <div className="mt-4 space-y-2">
        {COPY.legend.bins.map((bin) => (
          <div key={bin.label} className="flex items-center gap-2 text-xs text-slate-300">
            <span className={`inline-block h-3 w-6 rounded ${bin.color}`} />
            <span>{bin.label}</span>
          </div>
        ))}
      </div>
